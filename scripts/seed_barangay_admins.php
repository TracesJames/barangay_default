<?php
/**
 * Create one barangay-scoped admin account per barangay.
...
foreach ($barangays as $row) {
    $result = barangay_create_admin_for_barangay($con, $row['id'], $row['barangay'], $defaultPassword);
    if ($result === null) {
        $skipped++;
        $existing = barangay_load_admin_account($con, (string) $row['id']);
        if ($existing) {
            $accounts[] = [
            ...
            echo "Skipped (has admin): {$row['barangay']} — {$existing['username']}\n";
            continue;
        }
...
$outFile = __DIR__ . '/barangay_admin_accounts.txt';
$lines = ["Barangay Admin Accounts (default password: $defaultPassword)", str_repeat('-', 60)];
