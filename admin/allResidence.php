...
try{
  ...
  $residenceListFilter = preg_replace('/[^a-z_]/', '', strtolower((string) ($_GET['filter'] ?? '')));
  $allowedResidenceFilters = ['voters', 'non_voters', 'children', 'senior', 'pwd', 'single_parent', 'indigenous'];
  if (!in_array($residenceListFilter, $allowedResidenceFilters, true)) {
      $residenceListFilter = '';
  }
...
<head>
  ...
  <style>
    ...
    .residence-filter-form .input-group-prepend .input-group-text {
      white-space: nowrap;
      font-size: 0.72rem;
      font-weight: 700;
    ...
    .residence-filter-form .form-control {
      min-width: 0;
    }

    .residence-filter-form select.form-control {
      ...
      <div class="card-body">
          <fieldset>
            <legend>NUMBER OF RESIDENCE <span id="total"></span></legend>
              <div class="mb-3">
                <a href="importResidence.php" class="btn btn-success elevation-2">
                  <i class="fas fa-file-excel"></i> Import from Excel
                </a>
                <a href="newResidence.php" class="btn btn-primary elevation-2">
                  <i class="fas fa-user-plus"></i> New Residence
                </a>
              </div>
              <div class="row residence-filter-form">
                <div class="col-sm-4">
                  <div class="input-group mb-3">
                    <div class="input-group-prepend">
                      <span class="input-group-text bg-indigo">CATEGORY</span>
                    </div>
                      <select name="category" id="category" class="form-control">
                        <option value="">--ALL RESIDENTS--</option>
                        <option value="children">CHILDREN (0-17)</option>
                        <option value="indigenous">INDIGENOUS (IP)</option>
                      </select>
                  </div>
                </div>
                <div class="col-sm-4">
...
<script>
  $(document).ready(function(){
    ...
    function applyResidenceFilterFromUrl() {
      var filter = <?= json_encode($residenceListFilter, JSON_UNESCAPED_UNICODE) ?>;
      $('#category').val('');
      $('#voters').val('');
      $('#senior').val('');
      $('#pwd').val('');
      ...
      switch (filter) {
        ...
        case 'pwd':
          ...
          break;
        case 'single_parent':
          $('#single_parent').val('YES');
          break;
        case 'indigenous':
          $('#category').val('indigenous');
          break;
      }
    }

    applyResidenceFilterFromUrl();
    filterData();
    editStatus();
    viewResidence();
    deleteResidence();
    ...
    $(document).on('click', '#search',function(){
      ...
      if(status != '' || voters != '' || age != '' || first_name != '' ||  middle_name != '' || last_name != '' || pwd != '' || senior != '' || resident_id != '' || single_parent != '' || purok != '' || category != ''){
