...
try{
  ...
  if(isset($_REQUEST['official_id'])){
    ...
    $restore_barangay_id = (string) ($row['barangay_id'] ?? '');
    if ($restore_barangay_id === '') {
      $restore_barangay_id = $activeBarangay['id'];
    }

    $row_check_position = barangay_get_position_row($con, $position);
    $check_position = strtoupper($row_check_position['position'] ?? '');

    if (!$row_check_position || barangay_position_limit_reached($con, $position, $restore_barangay_id)) {
      exit('error');
    }
    ...
    $sql_official_end_status = "INSERT INTO `official_status` (`official_id`, `barangay_id`, `status`, `senior`,`voters`, `position`,`date_undeleted`, `term_from`, `term_to`, `pwd`,`pwd_info`,`single_parent`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
    $stmt_official_end_status = $con->prepare($sql_official_end_status) or die ($con->error);
    $stmt_official_end_status->bind_param('ssssssssssss',$official_id_end,$restore_barangay_id,$status,$senior,$voters,$position,$date_undeleted,$term_from,$term_to,$pwd,$pwd_info,$single_parent);
