...
$termRange = official_parse_term_range((string) ($_POST['edit_term_years'] ?? ''));
if (!official_validate_term_range((string) ($_POST['edit_term_years'] ?? ''))) {
  exit('errorTermYear');
}
$edit_term_from = $con->real_escape_string($termRange['from']);
$edit_term_to = $con->real_escape_string($termRange['to']);
...
$edit_fathers_name = $con->real_escape_string($row_check_offical['fathers_name'] ?? '');
$edit_mothers_name = $con->real_escape_string($row_check_offical['mothers_name'] ?? '');
...
$official_barangay_id = (string) ($row_check_offical['barangay_id'] ?? '');
$row_position_limit = barangay_get_position_row($con, $edit_position);
$new_position = strtoupper($row_position_limit['position'] ?? '');

if (
    !$row_position_limit
    || barangay_position_limit_reached($con, $edit_position, $official_barangay_id, $official_id)
) {
  exit('error');
}
...
$stmt_official->close();
  ...
  if($_POST['edit_term_years_check'] == 'true' || $_POST['edit_term_years_check'] === TRUE){

  
    $date_activity = $now = date("j-n-Y g:i A");  
    $oldTerm = official_term_range_display($old_term_from, $old_term_to);
    $newTerm = official_term_range_display($edit_term_from, $edit_term_to);
    $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL TERM - '.' ' .$official_id.' |' .' '. ' FROM '.$oldTerm.' TO '. $newTerm;
    $status_activity_log = 'update';
    $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
    $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
    $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
